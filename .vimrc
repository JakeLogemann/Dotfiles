...
" Plugins {{{1
" Plugins: Initialize vim-plug; install if neccesary {{{2
let g:plug_src_path = expand('~/.vim/autoload/plug.vim')
...
let g:curl_bin = expand((has('win32')&&!has('win64'))? 'C:\Windows\Sysnative\curl.exe':'curl')
...
" Install & Load 3rd Party Plugins {{{2
...
" Include user's extra bundle(s) {{{2
...
set enc=utf-8 fenc=utf-8 fencs=utf-8
...
if exists('$SHELL') | set shell=$SHELL
else | set shell=/bin/sh | endif
...
augroup vimrc-edit
  ...
  autocmd BufEnter     $MYVIMRC :setl foldopen=all
  autocmd BufEnter     $MYVIMRC :setl foldclose=all
